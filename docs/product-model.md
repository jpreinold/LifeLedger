...
Guided tracking is orchestration, not a new persisted domain. No workflow ID or serialized workflow draft is stored on an item. A responsibility remains an ordinary editable reminder, a document remains a scanned record attachment, and the connection remains a user-scoped `reminder_for` link. Existing generic Add item, Add reminder, custom detail, protected detail, document, and related-item paths remain available.
