...
| --- | --- | --- | --- | --- |
| Item | A real-world thing the user wants to keep track of, such as a person, vehicle, pet, home, passport, policy, subscription, or warranty. | Existing `Record` model and record APIs. | The registry maps every `record_type`; Person has a display/preferred name, aliases, relationship context, month/day-or-full birthday, responsibilities, documents, and related Items. | Existing data must remain visible; `Record` stays internal. Guided item bundles may compose current APIs later, without per-type tables or routes. |
| Capture | An original statement LifeLedger has not yet safely organized. | Durable assistant repository row. | Text Capture remains in the Inbox through clarification, review, failure, completion, retry, or dismissal. | Future scoped clients create Captures; they never receive raw CRUD. |
| Action proposal | Reviewable changes inferred from one Capture. | Versioned `ActionProposal` with strict actions/results. | LifeLedger owns targets, risk, confirmation, expiry, idempotency, and execution. | Full assistant undo and merge/split correction are deferred. |
...
Every current backend record type has a registry definition. The generic `general` type and unknown runtime values present as “Other item,” without discarding stored details. Person is implemented on the generic Item persistence foundation. Provider remains a possible future type and is not silently mapped to unrelated fields.
