...
Phase 12 adds an append-only, user-visible ledger for responsibility creation, completion, renewal, snooze, due-date changes, reopen actions, and supporting documents. Current reminder state remains separate from history. Recurring cycles have deterministic occurrence IDs, duplicate submissions are idempotent, renewal preserves previous and accepted next dates, and reminder History plus item Activity load in bounded pages.
...
Reminder lifecycle state separates persisted current state from derived urgency and the dedicated history ledger. Persisted current fields include the meaningful `due_date`, completion metadata, optional `snoozed_until`, optional `archived_at`, current occurrence ID, and version. The old bounded `lifecycle_events` field remains read-compatible but is not the Phase 12 source of history. Derived statuses are calculated centrally as Completed, Overdue, Due today, Urgent, Upcoming, or Scheduled. The default windows are: overdue before the current local date, due today on the current local date, urgent within 7 calendar days, upcoming within 30 calendar days, and scheduled beyond 30 days. Date-only due dates stay date-only so local calendar dates do not shift through UTC conversion.

Snoozing sets `snoozed_until` and defers attention without changing `due_date`, renewal dates, expiration dates, or record dates. Clearing a snooze restores the effective attention date to the underlying important date. Renewing is distinct from completing: renewable reminders keep one reminder entity, append one durable `renewed` event with the previous and new dates, clear obsolete snooze/alert state, advance the current occurrence, and safely update the registered linked-item date when it is empty or still matches the accepted prior cycle.
...
Record detail opens as a compact dashboard with Overview, Responsibilities, Documents, Related items, and Activity tabs where useful. Overview shows populated essentials, important dates, additional details, and notes without blank rows or "not provided" placeholders. Activity aggregates only genuine connected responsibility events; Documents and Related items remain first-class tabs backed by the existing secure attachment and linked-item systems.
